...
@Injectable()
export class SignAdminStrategy extends PassportStrategy(Strategy, 'jwt-sign-admin') {
  constructor(
  ...
  async validate(request: Request, payload: any): Promise<Users> {
    ...
    const users = JSON.parse(tokenInfo) as Users;
    if (users.grade < Users.GRADE.ADMIN) {
      throw new ForbiddenException({
